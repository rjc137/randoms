...
import com.bitcoin.analyzer.model.Tweet;
...
public class Main {
    ...
    public static void main(String args[]) throws IOException, URISyntaxException, InterruptedException, ExecutionException {
        ...
        Tweets response = new Tweets();
        
        for (int i = 0; i < 1; i++) {
            LocalDateTime startDateTime = LocalDateTime.now();
            logger.info("Cycle {} : Starting to Pull Tweets At : {}", i, startDateTime.toString() );
            LocalDateTime finishDateTime = startDateTime.plusMinutes(1);
            
            response = search(startDateTime, finishDateTime, bundle.getString(keyword_key), bearerToken);
        ...
        CompletionService<TypeSentiment> compService = new ExecutorCompletionService<>(exec);
        
        
        
        for (Tweet item : response.getData()) {
            compService.submit(new SentimentAnalyzer(item.getText()));
            logger.info("Submitting tweet for analysis");
        }
        for(int i=0;i<response.getData().size();i++) {
            Future<TypeSentiment> future = compService.take();
            TypeSentiment sentiment = future.get(); // wait for a processor to complete
       ...
       /* for (Tweet status : response.getData()) {
    ...
    private static Tweets search(LocalDateTime startDateTime, LocalDateTime finishDateTime, String searchString, String bearerToken) throws IOException, URISyntaxException, InterruptedException {
        ...
        for (int i = 0; (i < max_attempts && LocalDateTime.now().isBefore(finishDateTime)); i++) {
            ...
            if (null != entity) {
                ...
                tweets.getData()
                    .addAll(resp.getData());
                logger.info("Total Tweets : {}", tweets.getData().size());
    ...
    private static URIBuilder getUriBuilder(LocalDateTime startDateTime) throws URISyntaxException {
        ...
        queryParameters.add(new BasicNameValuePair("start_time", toRFC3339(startDateTime.minusHours(24L))));
...
class SentimentAnalyzer implements Callable<TypeSentiment> {
    private String text;
    ...
    @Override
    public TypeSentiment call() throws Exception {
